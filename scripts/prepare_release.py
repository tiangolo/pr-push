"""Prepare a release by updating the project version, README, and release notes."""
...
README_ACTION_REF_PATTERN = re.compile(r"(?m)(tiangolo/pr-push@)(\d+\.\d+\.\d+)")
...
def update_readme(
    ...
    def replace_ref(match: re.Match[str]) -> str:
        ...
        if matched_version != current_version:
            raise RuntimeError(
                f"Found README PR Push version {matched_version}, expected {current_version}"
            )
        replacements += 1
    ...
    if replacements == 0:
        raise RuntimeError(f"Could not find PR Push action refs in {readme_file}")
    return new_content
